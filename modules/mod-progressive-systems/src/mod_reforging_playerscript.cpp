...
public:
    ...
    void OnPlayerApplyItemModsBefore(Player* player, uint8 slot, bool apply, uint8 itemProtoStatNumber, uint32 statType, int32& val) override
    {
        ...
        {
            // NOTE: Reforging stat increase is handled via HandleStatModifier (flat modifier)
            // This ensures proper integration with Item Upgrade system
            ...
            // Decrease the stat value (this happens BEFORE Item Upgrade multipliers)
            // Order: Base stat -> Reforging (flat decrease) -> Item Upgrade (multiplier)
