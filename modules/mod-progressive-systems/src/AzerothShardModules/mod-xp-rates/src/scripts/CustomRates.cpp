...
float CustomRates::GetRateFromDB(const Player *player) {
    QueryResult result = CharacterDatabase.Query("SELECT xp_rate FROM character_xp_rate WHERE guid = %d", player->GetGUID().GetCounter());
...
void CustomRates::SaveRateToDB(const Player *player, float rate, bool update) {
    if (update) {
        CharacterDatabase.Execute("UPDATE character_xp_rate SET xp_rate = %f WHERE guid = %d", rate, player->GetGUID().GetCounter());
    } else {
        CharacterDatabase.Execute("INSERT INTO character_xp_rate (guid, xp_rate) VALUES (%d, %f)", player->GetGUID().GetCounter(), rate);
    }
}

void CustomRates::DeleteRateFromDB(ObjectGuid guid) {
    CharacterDatabase.Execute("DELETE FROM character_xp_rate WHERE guid = %d", guid.GetCounter());
}
