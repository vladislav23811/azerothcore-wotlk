...
void AzthPlayer::SetTimeWalkingLevel(uint32 itsTimeWalkingLevel, bool clearAuras/*=true*/, bool save /*=true*/, bool login/*=false*/)
{
    ...
    if (itsTimeWalkingLevel > 0)
    {
        ...
        if (save)
            CharacterDatabase.DirectExecute("INSERT IGNORE INTO azth_timewalking_characters_active (`id`, `level`) VALUES ('%d', '%d');", player->GetGUID().GetCounter(), sAZTH->GetAZTHPlayer(player)->GetTimeWalkingLevel());
    ...
    else
    {
        ...
        if (save) {
            CharacterDatabase.DirectExecute("DELETE FROM azth_timewalking_characters_active WHERE  `id`=%d;", player->GetGUID().GetCounter());
