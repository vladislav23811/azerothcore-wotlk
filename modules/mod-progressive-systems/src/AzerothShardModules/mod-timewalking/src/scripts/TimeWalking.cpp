...
public:
    ...
    void OnStartup()
    {
        ...
        QueryResult timewalking_table = ExtraDatabase.Query("SELECT id,name,exp,phase,level,bonus,criteria FROM timewalking ORDER BY exp, phase, level, name;");
        if (!timewalking_table)
        {
            LOG_INFO("module", ">> Loaded 0 raids for TimeWalking. DB table `timewalking` is empty.");
            return;
        }
        ...
        //-------------------------------------------------------------------
        //Loading levels
        QueryResult timewalkingLevel_table = ExtraDatabase.Query("SELECT level,race,class,"
        ...
        //-------------------------------------------------------------------
        //Loading achievement
        QueryResult azthAchievement_table = ExtraDatabase.Query("SELECT achievement,criteria,Points,category,parentCategory,difficulty,levelMax,levelMin,level,originalPoints,Name,Description,reward,rewardCount,killCredit,specialLevelReq,reqDimension FROM azth_achievements WHERE criteria != 0 ORDER BY achievement;");
        if (!azthAchievement_table)
        {
            LOG_INFO("module", ">> Loaded 0 achievements for TimeWalking. DB table `azth_achievements` is empty.\n");
            return;
        }
...
public:
    ...
    {
        QueryResult timewalkingCharactersActive_table = CharacterDatabase.Query("SELECT id,level FROM azth_timewalking_characters_active WHERE id = %d", player->GetGUID().GetCounter());
    ...
    void OnPlayerLogin(Player* player) override
    {
        ...
        if (rep > 0)
        {
            QueryResult last_activity_rep = CharacterDatabase.Query("SELECT c.logout_time,MAX(questEndTime),MAX(instanceEndTime),MAX(instanceStartTime) FROM azth_quest_log AS q LEFT JOIN characters AS c ON q.guid=c.guid WHERE q.guid = %d;", player->GetGUID().GetCounter());
    ...
    void OnAfterUpdateEncounterState(Map* map, EncounterCreditType /*type*/,  uint32 /*creditEntry*/, Unit* /*source*/, Difficulty difficulty_fixed, DungeonEncounterList const* encounters, uint32 dungeonCompleted, bool updated) override
    {
        ...
        for (size_t i = 0; i < list.size(); i++)
        {
            ...
            if (count >= total)
            {
                ...
                CharacterDatabase.DirectExecute("INSERT INTO azth_quest_log (guid, groupId, quest, dungeon, difficulty, sLevel, nLevel, gSize, instanceStartTime, instanceEndTime)"
