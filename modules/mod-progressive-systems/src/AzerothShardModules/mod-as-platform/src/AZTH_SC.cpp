...
// Custom 1v1 Arena constants (matching npc_arena1v1.cpp definitions)
#ifndef ARENA_TEAM_1v1
#define ARENA_TEAM_1v1 1
#define ARENA_TEAM_1V1 1  // Also support uppercase variant
#endif
#ifndef ARENA_SLOT_1v1
#define ARENA_SLOT_1v1 3
#define ARENA_SLOT_1V1 3  // Also support uppercase variant
#endif
#ifndef ARENA_TYPE_1v1
#define ARENA_TYPE_1v1 1
#define ARENA_TYPE_1V1 1  // Also support uppercase variant
#endif
#ifndef BATTLEGROUND_QUEUE_1v1
#define BATTLEGROUND_QUEUE_1v1 11
#define BATTLEGROUND_QUEUE_1V1 11  // Also support uppercase variant
#endif

// Custom 3v3 Solo Arena constants
#ifndef ARENA_TEAM_SOLO_3v3
#define ARENA_TEAM_SOLO_3v3 4
#endif
#ifndef ARENA_SLOT_SOLO_3v3
#define ARENA_SLOT_SOLO_3v3 3
#endif
#ifndef BATTLEGROUND_QUEUE_3v3_SOLO
#define BATTLEGROUND_QUEUE_3v3_SOLO 7
#endif
...
public:
    ...
    {
        // Temp arena teams (ID >= MAX_ARENA_TEAM_ID) should not be saved to database
...
public:
    ...
    // OnHandleDevCommand signature changed in base class: OnHandleDevCommand(Player* player, bool& enable)
    // Original implementation had: OnHandleDevCommand(Player* player, std::string& argstr)
    // This functionality needs to be re-implemented using the new signature if needed
    void OnHandleDevCommand(Player* player, bool& enable) override
    {
        // Implementation removed - original signature was different
        // If phase mask/gm toggling is needed, it should be done elsewhere
    }
};
...
public:
    ...
    void OnPlayerSave(Player* player) override
    {
        ...
        wowArmoryTrans->Append("DELETE FROM armory_character_stats WHERE guid = %u", player->GetGUID().GetCounter());
    ...
    bool OnPlayerCanBattleFieldPort(Player* player, uint8 arenaType, BattlegroundTypeId BGTypeID, uint8 action) override
    {
        ...
        // Check talents for 1v1 and 3v3 solo queues when accepting join
        if (action == 1 /*accept join*/)
        {
            // For 1v1, check if healing/tank talents are forbidden
            if (bgQueueTypeId == (BattlegroundQueueTypeId)BATTLEGROUND_QUEUE_1v1)
            {
            ...
            // For 3v3 solo, check using Solo3v3 method
            else if (bgQueueTypeId == (BattlegroundQueueTypeId)BATTLEGROUND_QUEUE_3v3_SOLO && !sSolo->Arena3v3CheckTalents(player))
            {
                return false;
            }
        }
...
public:
    ...
    void ValidateSpellAtCastSpellResult(Player* player, Unit* /* mover */, Spell* spell, uint32 oldSpellId, uint32 spellId) override
    {
        ...
        // Note: SetSpellInfo doesn't exist - m_spellInfo is const
        // The spell was already created with the correct SpellInfo from ValidateSpellAtCastSpell
        // If we need to send a result for oldSpellId, it should be handled differently
...
public:
    ...
    static bool HandleGameObjectAddGuildCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (GameObjectData const* goData = sObjectMgr->GetGameObjectData(db_lowGUID))
            sObjectMgr->AddGameobjectToGrid(db_lowGUID, goData);
    ...
    static bool HandleNpcAddGuildCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (guildhouseaddid == 2)
        {
            ...
            if (guildResult)
            {
                ...
                GHobj.UpdateGuardMap(creature->GetGUID().GetCounter(), fields[0].Get<int32>());
            }
        }
