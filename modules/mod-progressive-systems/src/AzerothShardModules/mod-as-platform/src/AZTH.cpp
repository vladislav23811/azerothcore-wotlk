...
// Custom 1v1 Arena constants (matching AZTH_SC.cpp definitions)
...
#endif
#ifndef ARENA_TYPE_3v3_SOLO
#define ARENA_TYPE_3v3_SOLO 5
#endif
...
{
    // PvP Ranks
    std::string pvprankStr = sConfigMgr->GetOption<std::string>("PvPRank.HKPerRank", "10,50,100,200,450,750,1300,2000,3500,6000,9500,15000,21000,30000");
    std::vector<std::string_view> PvPRankList = Acore::Tokenize(pvprankStr, ',', false);
    for (uint8 i = 0; i < PvPRankList.size() && i < HKRANKMAX; i++)
        _PvP_Ranks[i] = Acore::StringTo<uint32>(PvPRankList[i]).value_or(0);
...
void AZTH::AddAZTHPlayer(Player* player)
{
    ...
    {
        LOG_ERROR("server.loading", "AZTH::AddAZTHPlayer - _playersStore.count(player)");
        return;
    }
...
void AZTH::AddAZTHObject(Object* object)
{
    ...
    {
        LOG_ERROR("server.loading", "AZTH::AddAZTHObject - _objectsStore.count(object)");
        return;
    }
...
void AZTH::AddAZTHGroup(Group* group)
{
    ...
    {
        LOG_ERROR("server.loading", "AZTH::AddAZTHGroup - _groupStore.count(group)");
        return;
    }
...
void AZTH::AddAZTHInstanceSave(InstanceSave* instanceSave)
{
    ...
    {
        LOG_ERROR("server.loading", "AZTH::AddAZTHInstanceSave - _instanceSaveStore.count(instanceSave)");
        return;
    }
...
void AZTH::AddAZTHLoot(Loot* loot)
{
    ...
    {
        LOG_ERROR("server.loading", "AZTH::AddAZTHLoot - _lootStore.count(instanceSave)");
        return;
    }
...
AzthPlayer* AZTH::GetAZTHPlayer(Player* player)
{
    if (!_playersStore.count(player))
    {
        LOG_ERROR("server.loading", "AZTH::GetAZTHPlayer - !_playersStore.count(player)");
        return nullptr;
    }
...
AzthObject* AZTH::GetAZTHObject(Object* object)
{
    if (!_objectsStore.count(object))
    {
        LOG_ERROR("server.loading", "AZTH::GetAZTHObject - !_objectsStore.count(object)");
        return nullptr;
    }
...
AzthGroupMgr* AZTH::GetAZTHGroup(Group* group)
{
    ...
    if (!_groupStore.count(group))
    {
        LOG_ERROR("server.loading", "AZTH::GetAZTHGroup - !_groupStore.count(group)");
        return nullptr;
    }
...
AzthInstanceMgr* AZTH::GetAZTHInstanceSave(InstanceSave* instanceSave)
{
    if (!_instanceSaveStore.count(instanceSave))
    {
        LOG_ERROR("server.loading", "AZTH::GetAZTHInstanceSave - !_instanceSaveStore.count(instanceSave)");
        return nullptr;
    }
...
void AZTH::DeleteAZTHPlayer(Player* player)
{
    if (!_playersStore.count(player))
    {
        LOG_ERROR("server.loading", "AZTH::DeleteAZTHPlayer - !_playersStore.count(player)");
        return;
    }
...
void AZTH::DeleteAZTHObject(Object* object)
{
    if (!_objectsStore.count(object))
    {
        LOG_ERROR("server.loading", "AZTH::DeleteAZTHObject - !_objectsStore.count(object)");
        return;
    }
...
void AZTH::DeleteAZTHGroup(Group* group)
{
    if (!_groupStore.count(group))
    {
        LOG_ERROR("server.loading", "AZTH::DeleteAZTHGroup - !_groupStore.count(group)");
        return;
    }
...
void AZTH::DeleteAZTHInstanceSave(InstanceSave* instanceSave)
{
    if (!_instanceSaveStore.count(instanceSave))
    {
        LOG_ERROR("server.loading", "AZTH::DeleteAZTHInstanceSave - !_instanceSaveStore.count(instanceSave)");
        return;
    }
...
void AZTH::DeleteAZTHLoot(Loot* loot)
{
    if (!_lootStore.count(loot))
    {
        LOG_ERROR("server.loading", "AZTH::DeleteAZTHLoot - !_instanceSaveStore.count(loot)");
        return;
    }
...
void AZTH::SendExternalMails()
{
    ...
    // if (!result)
    // {
    //     LOG_ERROR("server.loading",("EXTERNAL MAIL> No mails in queue...");
    //     return;
    // }
    ...
    //         }
    //         else
    //             LOG_ERROR("server.loading",("non-existing item");
    //     }
    ...
    // LOG_ERROR("server.loading",("EXTERNAL MAIL> ALL MAILS SENT!");
}
