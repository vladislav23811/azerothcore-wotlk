...
void AzthPlayer::removeSmartStoneCommand(SmartStonePlayerCommand command,
    ...
    if (query) {
        CharacterDatabase.DirectExecute("DELETE FROM `character_smartstone_commands` "
...
bool AzthPlayer::BuySmartStoneCommand(ObjectGuid vendorguid, uint32 vendorslot,
    ...
    if (!player->IsGameMaster() &&
            ((pProto->HasFlag2(ITEM_FLAG2_FACTION_HORDE) &&
            player->GetTeamId(true) == TEAM_ALLIANCE) ||
            (pProto->HasFlag2(ITEM_FLAG2_FACTION_ALLIANCE) &&
    ...
    if (crItem->ExtendedCost) {
        ...
        if (!iece) {
            LOG_ERROR("server.loading", "Item %u have wrong ExtendedCost field value %u",
            ...
            0) // Assume price cannot be negative (do not know why it is int32)
    {
        ...
        if ((uint32) count > maxCount) {
            LOG_ERROR("server.loading", "Player %s tried to buy %u item id %u, causing overflow",
    ...
    if (!sSmartStone->isNullCommand(command)) {
        ...
        sScriptMgr->OnPlayerBeforeStoreOrEquipNewItem(player, vendorslot, item, count, bag, slot, pProto, creature, crItem, false);
    }
...
std::map<uint32,WorldLocation> AzthPlayer::getLastPositionInfoFromDB() {
    ...
    do
    {
        ...
        uint16 mapId = posFields[1].Get<uint16>();
        lastPos[type] = WorldLocation(mapId, posFields[2].Get<float>(), posFields[3].Get<float>(), posFields[4].Get<float>(), player->GetOrientation());
...
void AzthPlayer::saveLastPositionInfoToDB(Player *pl) {
    ...
    for ( it = lastPositionInfo.begin(); it != lastPositionInfo.end(); it++ ) {
        ...
        trans->Append("REPLACE INTO character_saved_position(charGuid,type,posX,posY,posZ,mapId) VALUES (%u, %u, %f, %f, %f, %u);", pl->GetGUID().GetCounter(), it->first, _loc.GetPositionX(), _loc.GetPositionY(), _loc.GetPositionZ(), _loc.GetMapId());
    }
