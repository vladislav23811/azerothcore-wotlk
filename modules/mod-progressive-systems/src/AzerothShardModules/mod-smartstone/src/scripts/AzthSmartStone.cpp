...
public:
                ...
                action >= 2000) // azeroth store
        {
            switch (action) {
                ...
                case 99999:
                    break;
                default:
                    LOG_ERROR("server.loading", "Smartstone: unhandled command! ID: %u, player GUID: %lu",
        ...
        if (selectedCommand.type == DO_SCRIPTED_ACTION_WITH_CODE || action == 2000) // azeroth store
        {
            switch (action) {
                ...
                case 99999:
                    break;
                default:
                    LOG_ERROR("server.loading", "Smartstone: unhandled command with code! ID: %u, player GUID: %lu", action, player->GetGUID().GetRawValue());
                    break;
            }
            if (selectedCommand.charges > 0) {
    ...
    bool OnUse(Player *player, Item *item, SpellCastTargets const & /*targets*/) override
    {
        ...
        if (parent == 1) // not-to-buy commands for the main menu
        {
            ...
            if (!sAZTH->GetAZTHPlayer(player)->isPvP())
                AddGossipItemFor(player, 0, "Read Me", GOSSIP_SENDER_MAIN, SMRTST_README_CHILD);
...
void SmartStone::loadCommands() {
    ...
    QueryResult ssCommandsResult = ExtraDatabase.Query(
    ...
    if (ssCommandsResult) {
        do {
            ...
            command.text_it = (*ssCommandsResult)[2].Get<std::string>();
            command.item = (*ssCommandsResult)[3].Get<uint32>();
            command.icon = (*ssCommandsResult)[4].Get<uint32>();
            command.parent_menu = (*ssCommandsResult)[5].Get<uint32>();
            command.type = (*ssCommandsResult)[6].Get<uint32>();
            command.action = (*ssCommandsResult)[7].Get<uint32>();
            command.charges = (*ssCommandsResult)[8].Get<int32>();
            command.duration = (*ssCommandsResult)[9].Get<uint64>();
    ...
    LOG_INFO("server.loading", "Smartstone: loaded %u commands", count);
}
        ...
        if (VendorItem const *item = items->GetItem(slot)) {
            if (ItemTemplate const *itemTemplate =
                ...
                if (!session->GetPlayer()->IsGameMaster() &&
                        ((itemTemplate->HasFlag2(ITEM_FLAG2_FACTION_HORDE) &&
                        session->GetPlayer()->GetTeamId() == TEAM_ALLIANCE) ||
                        (itemTemplate->HasFlag2(ITEM_FLAG2_FACTION_ALLIANCE) &&
