    ...
    for (uint8 slot = 0; slot < itemCount; ++slot)
    {
        if (VendorItem const* item = items->GetItem(slot))
        {
            if (ItemTemplate const* itemTemplate = sObjectMgr->GetItemTemplate(item->item))
            {
                ...
                if (!session->GetPlayer()->IsGameMaster() && ((itemTemplate->HasFlag2(ITEM_FLAG2_FACTION_HORDE) && session->GetPlayer()->GetTeamId() == TEAM_ALLIANCE) || (itemTemplate->HasFlag2(ITEM_FLAG2_FACTION_ALLIANCE) && session->GetPlayer()->GetTeamId() == TEAM_HORDE)))
                    continue;
                ...
                {
                    LOG_ERROR("server.loading", "SendListInventory: conditions not met for creature entry %u item %u", vendor->GetEntry(), item->item);
                    continue;
                }
...
bool HearthstoneMode::PlayerCanUseItem(Item const* item, Player* player, bool classCheck)
{
    ...
    if (proto)
    {
        if (proto->HasFlag2(ITEM_FLAG2_FACTION_HORDE) && player->GetTeamId(true) != TEAM_HORDE)
            return false;

        if (proto->HasFlag2(ITEM_FLAG2_FACTION_ALLIANCE) && player->GetTeamId(true) != TEAM_ALLIANCE)
            return false;
...
void HearthstoneMode::loadHearthstone()
{
    ...
    QueryResult hsAchiResult = ExtraDatabase.Query("SELECT data0, data1, creature, type FROM hearthstone_criteria_credits");
    ...
    LOG_INFO("server.loading", "Hearthstone Mode: loaded %u achievement definitions", count);
    ...
    LOG_INFO("server.loading", "Hearthstone Mode: loaded %u transmog items", itemCount);
        ...
        QueryResult hsQuestResult = ExtraDatabase.Query("SELECT id, flag, specialLevel, reqDimension, groupLimit, startTime, endTime FROM hearthstone_quests");
    ...
    QueryResult hsVendorResult = ExtraDatabase.Query("SELECT vendorId, reputation, `value`, gossipSatisfied, gossipUnsatisfied, PvPVendor FROM reputation_vendor");
