...
public:
    ...
    void OnStartup() override
    {
        ...
        std::istringstream(std::string(arena_timestamp_table[2].Get<std::string>())) >> enable;
...
public:
    ...
    static bool HandleSetMaxItemLevelCommand(ChatHandler* handler, Optional<uint32> itemLevel)
    {
        ...
        uint32 levelValue = *itemLevel;

        // Check for -1 (reset) - Optional doesn't handle -1 directly, so we need special handling
        // Since Optional<uint32> can't represent -1 properly, we'll use a different approach
        if (levelValue == UINT32_MAX)  // Using max value as reset signal
        {
        ...
        {
            sASeasonMgr->SetItemLevel(levelValue);
