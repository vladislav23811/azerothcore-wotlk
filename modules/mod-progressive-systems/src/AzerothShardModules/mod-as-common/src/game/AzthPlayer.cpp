...
void AzthPlayer::CreateWowarmoryFeed(uint32 type, uint32 data, uint32 item_guid, uint32 item_quality)
{
    ...
    */
    ...
    {
        LOG_ERROR("server.loading", "[Wowarmory]: player is not initialized, unable to create log entry!");
        return;
    }

    if (type <= 0 || type > 3)
    {
        LOG_ERROR("server.loading", "[Wowarmory]: unknown feed type: %d, ignore.", type);
        return;
    }

    if (data == 0)
    {
        LOG_ERROR("server.loading", "[Wowarmory]: empty data (GUID: %u), ignore.", player->GetGUID().GetCounter());
        return;
    }
    ...
    LOG_DEBUG("server.loading", "[Wowarmory]: create wowarmory feed (GUID: %u, type: %d, data: %u).", feed.guid, feed.type, feed.data);
...
// Temp Gear
void AzthPlayer::SetTempGear(bool hasGear)
{
    PVPTempGear = hasGear;
}
