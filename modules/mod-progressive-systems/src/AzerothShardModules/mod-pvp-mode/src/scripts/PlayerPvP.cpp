...
void AzthPlayer::loadPvPInfo() {
    ...
    QueryResult isPvPAccount = CharacterDatabase.Query("SELECT isPvP FROM azth_pvp_accounts WHERE id = '%d';", accId);
    ...
    if (!this->m_isPvP) {
        ...
        QueryResult isPvPCharacter = CharacterDatabase.Query("SELECT isPvP FROM azth_pvp_characters WHERE id = '%d';", characterId);
