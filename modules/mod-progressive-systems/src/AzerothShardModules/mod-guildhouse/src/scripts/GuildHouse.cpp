...
bool GuildHouseObject::ChangeGuildHouse(uint32 guild_id, uint32 newid)
{
    if (newid == 0) // Vendi
    {
        ...
        LOG_INFO("server.loading", "Guild {} sold guild house {}! ", guild_id, itr->second.Id);
        ExtraDatabase.DirectExecute("UPDATE `guildhouses` SET `guildId` = 0 WHERE `guildId` = {}", guild_id);
    ...
    else // Compra
    {
        ...
        QueryResult result = ExtraDatabase.Query("SELECT `x`, `y`, `z`, `map`, `minguildsize`, `price` FROM `guildhouses` WHERE `id` = {}", newid);
        if (!result)
        ...
        QueryResult addResult = CharacterDatabase.Query("SELECT `GuildHouse_Add` FROM `gh_guildadd` WHERE `guildId` = {}", guild_id);
        if (addResult)
        {
            Field *addFields = addResult->Fetch();
            add = addFields[0].Get<uint32>();
        }
        else
        {
            CharacterDatabase.DirectExecute("INSERT INTO `gh_guildadd` (`guildId`,`GuildHouse_Add`) VALUES ( {} , 1 )", guild_id);
        }

        GuildHouse NewGH(guild_id, id, x, y, z, map, minguildsize, price, add);
        GH_map[guild_id] = NewGH;

        LOG_INFO("server.loading", "Guild {} bought guild house {}! ", guild_id, newid);
        ExtraDatabase.DirectExecute("UPDATE `guildhouses` SET `guildId` = {} WHERE `id` = {}", guild_id, newid);
...
void GuildHouseObject::LoadGuildHouse()
{
    ...
    // if (!result)
    // {
    //     LOG_ERROR("server.loading", "Loaded 0 Guildhouses - NO RESULT FROM QUERY");
    //     return;
    // }
	...
	// 		{
    //             LOG_ERROR("server.loading", "Guild %u does not exist - guildhouse not loaded", guildID);
	// 			continue;
	// 		}


    //         QueryResult result2 = CharacterDatabase.Query("SELECT `GuildHouse_Add` FROM `gh_guildadd` WHERE `guildId` = %u", guildID);
    //         if (result2)
    //         {
    //             LOG_INFO("server.loading", "Loaded guildadd information for guild %u", guildID);
	...
	// 	else
    //     {
    //         LOG_ERROR("server.loading", "Loading GH for guild %u failed - NOT VALID", guildID);
    ...
    LOG_INFO("server.loading", "Loaded  %lu Guildhouses", GH_map.size());
}
...
void GuildHouseObject::LoadGuildHouseAdd()
{
    ...
    LOG_INFO("server.loading", "Loading GuildHouse npcs - objects...");
    ...
    if (!result)
    {
        ...
        LOG_ERROR("server.loading", "Loaded 0 guildhouse npcs - objects");
        return;
    }
    ...
    do
    {
        ...
        if (type == CREATURE)
        {
            ...
            {
                LOG_ERROR("server.loading", "Data for creature %u not present", guid);
                continue;
            }
        ...
        else if (type == OBJECT)
        {
            ...
            {
                LOG_ERROR("server.loading", "Data for gameobject %u not present", guid);
                continue;
            }
    ...
    LOG_INFO("server.loading", "Loaded  %lu Guildhouse objects", GH_AddHouse.size());
}
    ...
    for (GuildHouseMap::iterator itr = GH_map.begin(); itr != GH_map.end(); itr++)
    {
        if (Guild* pGuild = sGuildMgr->GetGuildById((*itr).first))
            if (pGuild->GetMemberSize() < (*itr).second.min_member)
            {
                ...
                LOG_INFO("server.loading", "GuildHouse %u set to 0 because guild %u has low members ( < %u )", (*itr).first, pGuild->GetId(), (*itr).second.min_member );
            }
    }
}
